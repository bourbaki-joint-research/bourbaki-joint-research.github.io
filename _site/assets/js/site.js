/* Bourbaki archive Ñ search page rendering and listing filter.
...
(function () {
  ...
  function safeCut(text, n) {
    ...
    var opens = (cut.match(/\\\(/g) || []).length;
    var closes = (cut.match(/\\\)/g) || []).length;
  ...
  function renderSearch() {
    ...
    host.innerHTML = hits.map(function (p) {
      var brief = safeCut(p.abstract, 280);
      var cut = brief.length < p.abstract.length;
      var shortA = esc(brief) + (cut ? "&hellip;" : "");
    ...
    host.querySelectorAll(".toggle").forEach(function (a) {
      a.addEventListener("click", function (ev) {
        ...
        /* ?? ???? ??? ?? ???? ?? ????. */
        if (window.MathJax && window.MathJax.typesetPromise) window.MathJax.typesetPromise([body]);
      });
    });
